...
* a look at it) that the underlying buffer is an array of the assumed type.
* Even then, BufferTyped can't prevent you from hurting yourself. It assumes
* that all bytes within [address,address+size] are of a given type that is
* defined through go generics. When you know that this is not the case, e.g.,
* because the buffer is of mixed types or because type consistency only starts
* at a certain offset from the buffer's address, then you are advised to use
* Skip or Child to jump to the correct address or to cast directly to a Go
* struct.
*
* Being a buffer of a given type, BufferTyped knows when to apply
* padding in order to meet the alignment of OpenGL std430 arrays.
 */
...
// Needless to say, using this the created buffer is 'unsafe'.
...
func BufferSet[T BufferType](b *BufferTyped, index uint32, t T) error {
...
// Return the index-th value of type T. This assumes that the buffer is an
// array of elements of type T.
func BufferGet[T BufferType](b *BufferTyped, index uint32) (t T, err error) {
...
// Return the buffer as a typed iterator. This assumes that the buffer is an
// array of elements of type T.
func BufferAsT[T BufferType](b *BufferTyped) iter.Seq2[uint32, T] {
...
// Create a new buffer for the given slice, accounting for OpenGL-specific
// array strides
func SliceAsBuffer[T BufferType](slice []T) *BufferTyped {
	if len(slice) == 0 {
		return nil
	}
